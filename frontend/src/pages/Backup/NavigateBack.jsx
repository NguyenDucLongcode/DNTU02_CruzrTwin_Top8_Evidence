...
import {
  ...
  ChevronRight, ArrowRight, FileText, X, Play
} from 'lucide-react';
...
const BUTTON_CONFIGS = [
  ...
  { id: 'btn_alarm_off', slot: '11', title: 'Tắt Chuông (OFF)', desc: 'Ngắt còi Alarm', icon: StopCircle, color: 'from-zinc-600/30 to-zinc-900/50 border-zinc-400/80 hover:border-zinc-300 text-gray-cool-300 shadow-[0_0_15px_rgba(161,161,170,0.3)]' },
    ...
    <div className="w-screen min-h-dvh bg-surface text-white font-mono flex flex-col justify-between p-6 lg:p-8 select-none relative">
      ...
      <header className="flex items-center justify-between z-10 bg-surface-panel/90 p-4 rounded-2xl border border-gray-cool-700/80 shadow-control-strong backdrop-blur-md">
        ...
        <div className="flex items-center gap-2.5">
          ...
          <button
            ...
            className={`px-3 py-1.5 rounded-xl border text-xs font-bold transition-elegant flex items-center gap-1.5 shadow-control-strong ${isServerRunning
          ...
          <button
            ...
            className={`px-3 py-1.5 rounded-xl border text-xs font-bold transition-elegant flex items-center justify-center gap-1.5 shadow-control-strong w-[130px] ${isRestarting
      ...
      <main className="flex-1 my-4 grid grid-cols-4 grid-rows-3 gap-4 z-10">
        {BUTTON_CONFIGS.map((btn) => {
          ...
          if (btn.id === 'btn_9') {
            return (
              <div key={btn.id} className="relative h-full grid grid-cols-2 gap-2">
                {/* Nửa trái: Undo */}
                <button
                  onClick={() => executeRunScriptApi(btn, 'undo')}
                  disabled={isLoading}
                  className={`w-full relative group rounded-2xl p-3 border bg-gradient-to-br transition-elegant cursor-pointer flex flex-col justify-between shadow-control-strong ${isSuccess ? 'from-teal-600/30 to-teal-950/60 border-teal-400 text-teal-300 ring-2 ring-teal-400 shadow-[0_0_20px_rgba(20,184,166,0.5)] animate-pulse' : btn.color
                    } ${isActive ? 'scale-[0.98]' : 'hover:scale-[1.02]'}`}
                >
                  <div className="flex items-center justify-between w-full">
                    <span className={`px-2 py-0.5 text-[9px] font-bold rounded-md border ${isSuccess ? 'bg-teal-500/30 border-teal-400 text-teal-200' : 'bg-gray-cool-900/60 border-gray-cool-600/50 text-gray-cool-300'
                      }`}>
                      SLOT 09A
                    </span>
                    {isLoading ? (
            ...
            return (
              <div key={btn.id} className="relative h-full grid grid-cols-2 gap-2">
                ...
                <button
                  ...
                  disabled={isLoading}
                  className={`w-full relative group rounded-2xl p-3 border bg-gradient-to-br transition-elegant cursor-pointer flex flex-col justify-between shadow-control-strong ${isSuccess ? 'from-emerald-600/30 to-emerald-950/60 border-emerald-400 text-emerald-300 ring-2 ring-emerald-400 shadow-[0_0_20px_rgba(16,185,129,0.5)] animate-pulse' : displayColor
                    } ${isActive ? 'scale-[0.98]' : 'hover:scale-[1.02]'}`}
                >
                  <div className="flex items-center justify-between w-full">
                    <span className={`px-2 py-0.5 text-[9px] font-bold rounded-md border ${isSuccess ? 'bg-emerald-500/30 border-emerald-400 text-emerald-200' : 'bg-gray-cool-900/60 border-gray-cool-600/50 text-gray-cool-300'
                      }`}>
                      SLOT 01A
                    </span>
                    {isLoading ? (
            ...
            return (
              <div key={btn.id} className="relative h-full grid grid-rows-2 gap-1.5 p-1 bg-surface panel-alternative border border-gray-cool-800/50 rounded-2xl shadow-[0_0_15px_rgba(6,182,212,0.10)]">
                <div className="grid grid-cols-3 gap-1.5">
                  ...
                  >
                    <span className="px-1.5 py-0.5 text-[8px] font-bold rounded border bg-gray-cool-900/60 border-gray-cool-700/50 w-fit">2A. SENSOR</span>
            ...
            return (
              <div key={btn.id} className="relative h-full grid grid-cols-2 gap-2">
                ...
                <button
                  ...
                  className={`w-full relative group rounded-2xl p-3 border bg-gradient-to-br transition-elegant cursor-pointer flex flex-col justify-between shadow-control-strong ${isSuccessOn ? 'from-rose-600/30 to-rose-950/60 border-rose-400 text-rose-300 ring-2 ring-rose-400 shadow-[0_0_20px_rgba(244,63,94,0.5)] animate-pulse' : btn.color
                    } ${isActiveOn ? 'scale-[0.98]' : 'hover:scale-[1.02]'}`}
                >
                  <div className="flex items-center justify-between w-full">
                    <span className={`px-2 py-0.5 text-[9px] font-bold rounded-md border ${isSuccessOn ? 'bg-rose-500/30 border-rose-400 text-rose-200' : 'bg-gray-cool-900/60 border-gray-cool-600/50 text-gray-cool-300'
                      }`}>
                      SLOT 11A
                    </span>
                    {isLoadingOn ? (
                ...
                <button
                  ...
                  disabled={loadingBtn !== null}
                  className={`w-full relative group rounded-2xl p-3 border bg-gradient-to-br transition-elegant cursor-pointer flex flex-col justify-between shadow-control-strong ${isSuccessOff ? 'from-zinc-500/30 to-zinc-800/60 border-zinc-400 text-gray-cool-300 ring-2 ring-zinc-400 shadow-[0_0_20px_rgba(161,161,170,0.5)] animate-pulse' : btnAlarmOff.color
                    } ${isActiveOff ? 'scale-[0.98]' : 'hover:scale-[1.02]'}`}
                >
                  <div className="flex items-center justify-between w-full">
                    <span className={`px-2 py-0.5 text-[9px] font-bold rounded-md border ${isSuccessOff ? 'bg-gray-cool-500/30 border-gray-cool-400 text-gray-cool-200' : 'bg-gray-cool-900/60 border-gray-cool-600/50 text-gray-cool-300'
                      }`}>
                      SLOT 11B
                    </span>
                    {isLoadingOff ? (
            ...
            return (
              <div key={btn.id} className="relative h-full grid grid-cols-2 gap-2">
                ...
                <button
                  onClick={() => handleTriggerScript(btn)}
                  disabled={loadingBtn !== null}
                  className={`w-full relative group rounded-2xl p-3 border bg-gradient-to-br transition-elegant cursor-pointer flex flex-col justify-between shadow-control-strong ${isSuccess10 ? 'from-emerald-600/30 to-emerald-950/60 border-emerald-400 text-emerald-300 ring-2 ring-emerald-400 shadow-[0_0_20px_rgba(16,185,129,0.5)] animate-pulse' : btn.color
                    } ${isActive10 ? 'scale-[0.98]' : 'hover:scale-[1.02]'}`}
                >
                  <div className="flex items-center justify-between w-full">
                    <span className={`px-2 py-0.5 text-[9px] font-bold rounded-md border ${isSuccess10 ? 'bg-emerald-500/30 border-emerald-400 text-emerald-200' : 'bg-gray-cool-900/60 border-gray-cool-600/50 text-gray-cool-300'
                      }`}>
                      SLOT 10A
                    </span>
                    {isLoading10 ? (
                ...
                <button
                  ...
                  disabled={loadingBtn !== null}
                  className={`w-full relative group rounded-2xl p-3 border bg-gradient-to-br transition-elegant cursor-pointer flex flex-col justify-between shadow-control-strong ${isSuccess11 ? 'from-emerald-600/30 to-emerald-950/60 border-emerald-400 text-emerald-300 ring-2 ring-emerald-400 shadow-[0_0_20px_rgba(16,185,129,0.5)] animate-pulse' : btn11.color
                    } ${isActive11 ? 'scale-[0.98]' : 'hover:scale-[1.02]'}`}
                >
                  <div className="flex items-center justify-between w-full">
                    <span className={`px-2 py-0.5 text-[9px] font-bold rounded-md border ${isSuccess11 ? 'bg-emerald-500/30 border-emerald-400 text-emerald-200' : 'bg-gray-cool-900/60 border-gray-cool-600/50 text-gray-cool-300'
                      }`}>
                      SLOT 10B
                    </span>
                    {isLoading11 ? (
          ...
          return (
            <div key={btn.id} className={`relative h-full ${openDropdown === btn.id ? 'z-[9999]' : 'z-10'}`}>
              <button
                ...
                className={`w-full h-full relative group rounded-2xl p-4 border bg-gradient-to-br transition-elegant cursor-pointer flex flex-col justify-between shadow-control-strong focus-visible:outline-2 focus-visible:outline-offset-2 active:scale-[0.97] ${isSuccess
              ...
              >
                ...
                <div className="flex items-center justify-between w-full">
                  <span className={`px-2.5 py-1 text-[10px] font-bold rounded-md border ${isSuccess || isBtn8Paused ? 'bg-success/30 border-success text-success-200' : 'bg-gray-cool-900/60 border-gray-cool-600/50 text-gray-cool-300'
                    }`}>
                    SLOT {btn.slot}
                  </span>
                  ...
                  {isSuccess ? (
                    ...
                    <span className={`w-2.5 h-2.5 rounded-full ${isLoading ? 'bg-warning animate-ping' : isActive ? 'bg-cyan-400 animate-pulse' : 'bg-gray-cool-600'}`} />
                  )}
                </div>
                ...
                <div className="flex items-center justify-between w-full pt-2 border-t border-gray-cool-700/50 text-[10px]">
                  <span className="text-gray-cool-500 font-mono">ID: {btn.id}</span>
                  <span className={`text-xs font-bold ${isSuccess ? 'text-success' : 'text-gray-cool-300'} group-hover:translate-x-1 transition-transform`}>
                    {btn.id === 'btn_9' && resetClickBadge ? (
              ...
              {btn.id === 'btn_4' && openDropdown === 'btn_4' && (
                <div className="absolute top-full left-0 mt-2 w-full bg-surface-panel border border-gray-cool-600/50 rounded-xl shadow-control-strong z-modal overflow-hidden flex flex-col">
                  <div className="bg-accent/20 px-3 py-1.5 text-[10px] font-bold text-accent border-b border-gray-cool-700/30 flex justify-between items-center">
                    ...
                    <button onClick={() => setOpenDropdown(null)} className="hover:text-white"><X className="w-3 h-3" /></button>
                  </div>
          ...
          {lastResponse ? (
            <div className="p-2 rounded bg-gray-cool-800/80 border border-gray-cool-700 flex flex-col gap-1">
              <div className="flex items-center justify-between text-cyan-300 font-bold border-b border-gray-cool-700/60 pb-1">
                ...
                <span className="text-gray-cool-500">{lastResponse.time}</span>
              </div>
            ...
            <div className="text-gray-cool-500 italic py-2 text-center">
              <Lightbulb className="w-5 h-5 inline mr-1 text-accent" /> Chưa có lệnh nào được thực thi. Hãy nhấp vào 1 trong 12 nút bấm phía trên để kiểm tra kết nối Robot hoặc kích chạy kịch bản.
            </div>
          )}
          ...
          {commandHistory.length > 0 && (
            <div className="pt-2 border-t border-gray-cool-700/60 space-y-1">
              ...
              {commandHistory.slice(0, 5).map((item, idx) => (
                <div key={idx} className="flex items-center justify-between text-[10px] px-2 py-1 rounded bg-gray-cool-800/60 border border-gray-cool-700/50 text-gray-cool-400">
                  <span className="flex items-center gap-2">
                    <span className="text-gray-cool-500">[{item.timestamp}]</span>
                    <span className="text-cyan-400 font-bold">{item.button_id}</span>
                    <span className="text-gray-cool-300">({item.name})</span>
                  </span>
                  <span className="text-success font-mono truncate max-w-md tabular-nums">{item.details || item.status}</span>
                </div>
              ))}
            </div>
          )}
