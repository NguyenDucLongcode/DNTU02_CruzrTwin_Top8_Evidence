...
function getLatestRoomLog(logs, zoneId, predicate) {
  ...
  const scopedLogs = zoneId
    ? logs.filter((log) => logMatchesRoom(log, zoneId))
    : logs;
...
function RoomDetailBottomBar({ roomId, floorIdx, sensorData }) {
  const sensor =
    sensorData[roomId] || sensorData[roomId?.replace(/L\d+/, "L1")] || {};
  ...
  const co2 = Number(
    sensor.co2 !== undefined
      ? sensor.co2
      : sensor.air_quality_or_co2 !== undefined
        ? sensor.air_quality_or_co2
        : 400.0,
  );
  const deviceStatus = String(
    sensor.device_status || sensor.status || "NORMAL",
  ).toUpperCase();
  ...
  const isWarning =
    !isCritical &&
    (deviceStatus === "WARNING" || temp >= 32 || co2 >= 631 || smoke >= 0.5);
  ...
  return (
    <div className="w-full h-full p-1.5 grid grid-cols-3 gap-2 bg-zinc-950 font-mono text-white overflow-hidden">
      ...
      <div className="relative rounded-lg overflow-hidden border border-blue-500/30 bg-black/40 flex flex-col h-full">
        <div className="px-2 py-0.5 bg-blue-950/60 border-b border-blue-500/30 text-[10px] font-bold text-blue-400 flex items-center justify-between">
          <span className="flex items-center gap-1">
            <Building className="w-3 h-3" /> FLOOR {floorIdx || 1} VIEW
          </span>
      ...
      <div
        className={`relative rounded-lg overflow-hidden border ${isCritical ? "border-red-500/40 bg-red-950/20" : isWarning ? "border-amber-500/40 bg-amber-950/20" : "border-emerald-500/40 bg-emerald-950/20"} flex flex-col h-full`}
      >
        <div
          className={`px-2 py-0.5 border-b text-[10px] font-bold flex items-center justify-between ${isCritical ? "bg-red-950/60 border-red-500/30 text-red-400" : isWarning ? "bg-amber-950/60 border-amber-500/30 text-amber-400" : "bg-emerald-950/60 border-emerald-500/30 text-emerald-400"}`}
        >
          <span className="flex items-center gap-1">
            <Flame className="w-3 h-3" /> ROOM {roomId} 3D
          </span>
      ...
      <div className="relative rounded-lg border border-emerald-500/30 bg-zinc-900/90 p-2 flex flex-col justify-between text-[11px] h-full overflow-y-auto">
        <div className="font-bold text-emerald-400 border-b border-zinc-800 pb-1 flex justify-between items-center">
          <span className="flex items-center gap-1">
            <BarChart3 className="w-3 h-3" /> SENSOR TELEMETRY
          </span>
        ...
        <div className="space-y-1 my-1">
          <div className="flex justify-between items-center bg-zinc-950/80 px-2 py-0.5 rounded border border-zinc-800/60">
            ...
            <span
              className={`font-bold ${temp >= 40 ? "text-red-400 animate-pulse" : temp >= 32 ? "text-amber-400" : "text-emerald-400"}`}
            >
              {temp.toFixed(1)} °C
            </span>
          </div>
          <div className="flex justify-between items-center bg-zinc-950/80 px-2 py-0.5 rounded border border-zinc-800/60">
            <span className="text-zinc-400 flex items-center gap-1">
              <Wind className="w-3 h-3" /> Khói
            </span>
            <span
              className={`font-bold ${smoke >= 0.5 ? "text-red-400 animate-pulse" : "text-emerald-400"}`}
            >
              {smoke.toFixed(1)} %
            </span>
          </div>
          <div className="flex justify-between items-center bg-zinc-950/80 px-2 py-0.5 rounded border border-zinc-800/60">
            <span className="text-zinc-400 flex items-center gap-1">
              <Activity className="w-3 h-3" /> CO₂
            </span>
            <span
              className={`font-bold ${co2 >= 1000 ? "text-red-400 animate-pulse" : co2 >= 631 ? "text-amber-400" : "text-emerald-400"}`}
            >
              {co2.toFixed(0)} ppm
            </span>
          </div>
        ...
        <div className="flex justify-between items-center text-[10px] text-zinc-400 pt-1 border-t border-zinc-800/60">
          <span className="flex items-center gap-1">
            <Bot className="w-3 h-3" /> Robot:{" "}
            <strong className="text-cyan-400">DISPATCHED</strong>
          </span>
          <span className="flex items-center gap-1">
            Plug:{" "}
            <strong
              className={isCritical ? "text-red-400" : "text-emerald-400"}
            >
              {isCritical ? "POWER OFF" : "ON"}
            </strong>
          </span>
        </div>
      </div>
...
export default function Dashboard() {
  ...
  const latestRoomRobotAction = useMemo(
    () =>
      getLatestRoomLog(roomLogs.robot, selectedZoneId, (log) =>
        Boolean(log.robot_action_id),
      ),
  ...
  const handleStartWebhookServer = async () => {
    try {
      ...
      setAckStatus(err.message || "error");
    }
  };
  ...
  useEffect(() => {
    ...
    const severity = String(
      latest?.predicted_level ||
        latest?.severity ||
        latest?.device_status ||
        "",
    ).toUpperCase();
    const alertId =
      latest?.source_ai_event_id || latest?.alert_id || latest?.timestamp || "";
    ...
    if (
      ...
      alertId !== lastAlertIdRef.current
    ) {
  ...
  useEffect(() => {
    ...
    const handleMouseMove = (e) => {
      ...
      const newWidth = Math.max(
        320,
        Math.min(900, sidebarDragRef.current.startWidth - delta),
      );
  ...
  useEffect(() => {
    ...
    const handleMouseMove = (e) => {
      ...
      const newPercent = Math.max(
        ...
        Math.min(
          90,
          mapResizeRef.current.startPercent + (delta / viewportWidth) * 100,
        ),
      );
        ...
        <div className="flex-1 overflow-y-auto flex flex-col">
          {LOG_CONFIG.map(({ type, header }) => (
            <LogPanel
              ...
              emptyMessage={
                selectedZoneId
                  ? `No ${header} log for ${activeRoom.id}.`
                  : `No ${header} log.`
              }
            />
          ))}
        ...
        >
          ...
          >
            {focusPanel === "sensor" && (
              <LogPanel
                title="SENSORS"
                data={roomLogs.sensors}
                height="100%"
                emptyMessage="Chưa có dữ liệu Sensor..."
              />
            )}
            {focusPanel === "orion" && (
              <LogPanel
                ...
                data={roomLogs.state}
                height="100%"
                emptyMessage="Chưa có dữ liệu Orion..."
              />
            )}
            ...
            {focusPanel === "robot" && (
              <LogPanel
                ...
                data={roomLogs.robot}
                height="100%"
                emptyMessage="Chưa có dữ liệu Robot Action..."
              />
            )}
            {focusPanel === "ack" && (
              <LogPanel
                ...
                data={roomLogs.ack}
                height="100%"
                emptyMessage="Chưa có dữ liệu Operator Ack..."
              />
            )}
          </div>
        </div>
