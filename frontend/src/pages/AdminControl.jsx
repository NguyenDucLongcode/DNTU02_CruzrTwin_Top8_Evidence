...
export default function AdminControl() {
  const [status, setStatus] = useState('Ready to control device data...');
  ...
  const triggerAction = async (action, roomId) => {
    ...
    setStatus(`[${new Date().toLocaleTimeString()}] Intervening in device config for room ${roomId}...`);
    ...
    try {
      ...
      if (result.success) {
        setStatus(`[${new Date().toLocaleTimeString()}] ${result.message}\n⏳ Restarting device simulator (3-5 seconds)...`);
      } else {
        setStatus(`Error: ${result.message}`);
      }
    } catch (error) {
      setStatus(`Server connection error: ${error.message}`);
    }
  };
  ...
  return (
    <div className="min-h-screen bg-[#121212] text-white p-5 font-sans">
      <div className="max-w-6xl mx-auto bg-[#1e1e1e] p-8 rounded-xl shadow-2xl">
        <h1 className="text-center text-[#ff5252] text-3xl font-bold mb-2">INSTRUCTOR CONTROL PANEL</h1>
        <p className="text-center text-gray-400 mb-8 text-sm">Manage room statuses in real-time (Send commands via MQTT simulation)</p>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5 mb-8">
          {ROOMS.map(room => {
            ...
            return (
              ...
              >
                <h3 className="m-0 text-[#ddd] text-xl text-center border-b border-[#444] pb-2 mb-2">Room {room}</h3>
                <button 
